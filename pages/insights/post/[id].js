...
export async function getServerSideProps({ req, resolvedUrl, params, query }) {
  ...
  const deepLinkPath = lang
    ? `/insights/post/${id}?lang=${encodeURIComponent(lang)}`
    : `/insights/post/${id}`

  const ogImage = safeHttpUrl(query?.image) || DEFAULT_OG_IMAGE
  const ogTitle = safeString(query?.title) || 'ICOC Insight'
  const ogDescription =
    safeString(query?.description) || 'Open this insight in ICOC app'
...
export default function InsightDeepLinkPage({
  canonicalUrl,
  deepLinkPath,
  ogImage,
  ogTitle,
